...
s.connect((host,port)) #3
s.send(request.encode()) #4
...
# *****************************
saved_filename = os.path.join("/tmp", "image.jpg")
if not os.path.exists("/tmp"):
    os.makedirs("/tmp")

with open(saved_filename,'w+b') as image:
    response = bytearray()
    while True:
