...
public class ConsoleGame {
	...
	public void playGame() {
		...
		level.setupLevel(NUM_ROWS, NUM_COLS); //Sets up the level
		while(!level.passedLevel()) { //If the level isn't passed yet, the program will the player for spaces in a while loop
			System.out.println(level); //prints out the level
			System.out.println("Here's the next turn!"); //Reminds the user it's the next turn
			if (level.moveNumSpaces(getLocationFromUser(NUM_ROWS, NUM_COLS), getInteger("How many spaces do you want the vehicle to move? "))) {
				level.incrementMoves();
			}
			else {
				System.out.println("That vehicle can't be moved to that space, please try again!");
			}
		}
		
		if(level.passedLevel()) {
			System.out.println("Congratulations you passed the level!");
		}
		System.out.println(level);  //prints out the current level
	}
	...
	public static Space getLocationFromUser(int maxRows, int maxCols) {
		try {
			while (true) {
				...
				String r = consoleReader.readLine();
				Space s = convertStringToIntPair(r);
				if (isValidLocation(s, maxRows, maxCols)) return s;
				System.out.println("Sorry, but \"" + r + "\" isn't a valid response.  Please try again.");
				System.out.println();
			}
