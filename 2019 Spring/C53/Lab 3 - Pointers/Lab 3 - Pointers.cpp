/*File Name: Lab 3 - Pointers.cpp
Author: Trinity Caitlin Atayan Belen
Description: Every time the user makes an account, the program will count it*/
...
int main() {
	char choice;
	int balance = 0;
	SavingsAccount* accountPtr1 = nullptr; //pointer for savings account
	WalletAccount* accountPtr2 = nullptr; //pointer for wallet account

	cout << "Current number of accounts is: " << BankAccount::getAccountCount() << "\n" << endl;
	...
	else 
	{
		...
		cout << "\n";
		cout << "Do you want to open a savings account with 2% interest rate? (y/n) "; //asks for savings account
		cin >> choice;
		cout << "\n";
		if (choice == 'y') {
			...
			cin >> balance;
			cout << "\n";
			accountPtr1 = new SavingsAccount; //Creates a new SavingsAccount object and assigns it to savings pointer
		}

		cout << "Do you want to open a wallet account with 2% interest and $" << balance << " maximum capacity? (y/n) ";
		cin >> choice;
		cout << "\n";
		...
		if (choice == 'y') {
			...
			cin >> balance;
			cout << "\n";
			accountPtr2 = new WalletAccount; //Creates a new WalletAccount object and assigns it to wallet pointer
		}
	}
	
	cout << "Current number of accounts is: " << BankAccount::getAccountCount() << "\n" << endl;

	//Deletes the pointers so that there is no memory leaks
	delete accountPtr1;
	delete accountPtr2;
	system("pause");
}